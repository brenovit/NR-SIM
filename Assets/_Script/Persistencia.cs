...
namespace SQLiter
{
	public class Persistencia : MonoBehaviour
	{
		...
		private const string SQL_TABLE_ITEM = "item";
		private const string SQL_TABLE_PERGUNTA = "pergunta";
		private const string SQL_TABLE_QUIZ = "quiz";
		...
		private void SQLiteInit ()
		{
			...
			mCommand.CommandText = "SELECT name FROM sqlite_master WHERE name='" + SQL_TABLE_ITEM + "'";
			mReader = mCommand.ExecuteReader ();
			if (!mReader.Read ()) {
				Debug.Log ("SQLiter - Could not find SQLite table " + SQL_TABLE_ITEM);
			...
			if (mCreateNewTable) {
				Debug.Log ("SQLiter - Creating new SQLite table " + SQL_TABLE_ITEM);

				// insurance policy, drop table
				mCommand.CommandText = "DROP TABLE IF EXISTS " + SQL_TABLE_ITEM;
				mCommand.ExecuteNonQuery ();

				// create new - SQLite recommendation is to drop table, not clear it
				mSQLString = "CREATE TABLE IF NOT EXISTS " + SQL_TABLE_ITEM + " (" +
				...
				if (DebugMode)
					Debug.Log ("SQLiter - SQLite table " + SQL_TABLE_ITEM + " was found");
			}
		...
		public List<Item> GetAllItens ()
		{
			...
			// if you have a bunch of stuff, this is going to be inefficient and a pain.  it's just for testing/show
			mCommand.CommandText = "SELECT id, nome FROM " + SQL_TABLE_ITEM;
			mReader = mCommand.ExecuteReader ();
			while (mReader.Read ()) {
				Item item = new Item ();
		...
		public int QueryInt (string column, string value)
		{
			int sel = -1;
			mConnection.Open ();
			mCommand.CommandText = "SELECT " + column + " FROM " + SQL_TABLE_ITEM + " WHERE " + "nome" + "='" + value + "'";
			mReader = mCommand.ExecuteReader ();
			if (mReader.Read ())
